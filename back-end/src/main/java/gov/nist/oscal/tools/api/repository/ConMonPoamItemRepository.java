...
public interface ConMonPoamItemRepository extends JpaRepository<ConMonPoamItem, Long> {
    ...
    // Note: caller must pass :q as a non-null String (use "" for "no filter").
    // pgjdbc binds untyped null as bytea, which makes lower(coalesce(?, '')) collapse
    // to lower(bytea) and fail. Avoiding the null bind altogether sidesteps that.
    @Query("SELECT i FROM ConMonPoamItem i WHERE i.snapshot = :snapshot " +
           ...
           "             OR LOWER(COALESCE(i.externalId, '')) LIKE LOWER(CONCAT('%', :q, '%')))")
